...
(define (zaehlen_ho x xs)
  (foldl (lambda (a b)           ; (count (curry equal? x)...  
           (if (equal? a x)      ; (length (filter (curry equal? x) xs))
               (add1 b)        ;  //filter -> liste mit nur 3en ...dann Listenlänge ...
               b))
         0
         xs))
...
|#
;#() für Vektoren -> schnelleren Zugriff
...
(define (draw-board state [yoffset 10] [scene (empty-scene 400 400)])
  (if (null? state)
      scene
      (draw-row (car state)
                10
...
(define (next-cellstate nachbarn alive)
  (let ([anzahl-nachbarn (foldl (lambda (elem acc)
                                 (if elem
                                     (add1 acc)
                                     acc)) 0 nachbarn)])   ; -> zaehlen ...
    (if (or (and alive (equal? anzahl-nachbarn 2))   ;if überflüssig -> or -> wahrheitswert
...
(define (next-state state)
  (map (lambda (row y)
         (map (lambda (cell x)
                (next-cellstate(nachbarn cell state y x)))
              row 
              (range 30)))
       state
         (range 30)))
...
(define start     
  (list
      (list #f #f #f #f #f #f #f #f #f #f #f #f #f #f #f #f #f #f #f #f #f #f #f #f #f #f #f #f #f #f) ;' auch mgl., da keine Symbole
